...
//TODO remove, use Await.h
	...
	struct IAwaitOld{
		IAwaitOld( SRCE )ι:_sl{sl}{};
		IAwaitOld( string name, SRCE )ι:_sl{sl},_name{move(name)}{};
		virtual ~IAwaitOld()=0;
...
//		friend Task;
	...
	inline IAwaitOld::~IAwaitOld(){}

	//Holds a result variable for await_ready
	struct IAwaitCache : IAwaitOld{
		IAwaitCache( SRCE )ι:IAwaitOld{sl}{};
		IAwaitCache( string name, SRCE )ι:IAwaitOld{move(name), sl}{};
		//virtual ~IAwaitOld()=0;
		//α await_suspend( HCoroutine h )ι->void override{ IAwaitOld::await_suspend(h); }
		α await_resume()ι->AwaitResult override{
			if(_pPromise)
				_result = IAwaitOld::await_resume();
			else{
				//AwaitResume();
				_result.Push(_sl);
			}
	...
	Τ struct TPoolAwait final : IAwaitOld{//todo rename FromSyncAwait?
	...
	struct PoolAwait final : IAwaitOld{//todo rename FromSyncAwait?
		using base=IAwaitOld;
...
#ifdef unused
	Ŧ CallAwait( std::promise<up<T>> p, IAwaitOld&& a )->Task{
		//auto p = move( p_ );
	...
	Ŧ Future( IAwaitOld&& a )->std::future<up<T>>{
...
#endif
	Ξ BCallAwait( std::promise<bool>&& p_, IAwaitOld&& a )->Task{
		auto p = move( p_ );
		AwaitResult r = co_await a;
		if( r.HasBool() )
			p.set_value( r.Bool() );
	...
	Ξ BFuture( IAwaitOld&& a )->std::future<bool>{
	...
	Ξ CallVAwait( std::promise<void>&& p_, IAwaitOld&& a )->Task{
		auto p = move( p_ );
		AwaitResult r = co_await a;
		if( r.HasError() )
			p.set_exception( r.Error()->Ptr() );
	...
	Ξ VFuture( IAwaitOld&& a )->std::future<void>{
		std::promise<void> p;
	...
	Ŧ CallAwait( std::promise<sp<T>> p, IAwaitOld&& a )ι->Task{
		AwaitResult r = co_await a;
		ASSERT( !r.HasValue() )
		if( r.HasShared() )
	...
	Ŧ SFuture( IAwaitOld&& a )ι->std::future<sp<T>>{
		std::promise<sp<T>> p;
