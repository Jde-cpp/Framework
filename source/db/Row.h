...
namespace Jde::DB
{
	struct IRow
	{
		...
		β GetCIString( uint position, SRCE )Ε->CIString=0;
	...
	template<> Ξ IRow::Get<CIString>( uint position, SL sl )Ε->CIString{ return GetCIString(position, sl); }
