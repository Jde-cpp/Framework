...
namespace Jde::DB
{
	...
	struct Γ IDataSource : std::enable_shared_from_this<IDataSource>
	{
		...
		template<class K=uint,class V=string> α SelectEnum( sv tableName, SRCE )noexcept{ return SelectMap<K,V>( format("select id, name from {}", tableName), string{tableName}, sl ); }
		ẗ SelectEnumSync( sv tableName, SRCE )noexcept(false)->sp<flat_map<K,V>>{ ASSERT(tableName.size()); return SFuture<flat_map<K,V>>( SelectEnum<K,V>( tableName, sl) ).get(); }
