...
namespace Jde{
	constexpr std::array<sv,27> ELogTagStrings = {"none",
		"access", "app", "cache", "client", "exception", "externalLogger", "http", "io",
...
/*		if( globalTag==_availableTags->end() ){
		...
		for( let& level : LogLevelStrings() ){
			...
			for( let& v : levelTags ){
				if( let name = v.is_string() ? v.get_string() : sv{}; !name.empty() ){
					tagIds.push_back( SetTag(name, sinkTags, (ELogLevel)i) );
					_fileLogLevels.emplace( ToLogTags(name), (ELogLevel)i );
				}
			}
			++i;
			if( tagIds.size() )
	...
	α Logging::ShouldLog( const ExternalMessage& m )ι->bool{
		auto p = find_if( _fileTags, [&](let& x){return x.Id==m.Tag;} );
