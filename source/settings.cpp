...
namespace Jde{
	...
	jvalue _settings;
	α Settings::Value()ι->const jvalue&{ return _settings; }
	...
	α Settings::FindString( sv path )ι->optional<string>{
		auto setting = Json::FindString( _settings, path );
		if( setting ){
			std::regex regex( "\\$\\((.+?)\\)" );
			for(;;){
...
α FileStem()ι->string{
	...
	α Path()ι->fs::path{
		...
		let fileName = fs::path{ Jde::format("{}.json", FileStem()) };
	...
	α Settings::Load()ι->void{
		...
		try{
			...
			_settings = boost::json::parse( IO::FileUtilities::Load(settingsPath) );
